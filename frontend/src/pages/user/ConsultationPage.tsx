import React, { useState, useCallback, useEffect } from "react";
import { useAuth } from "@/context/AuthContext";
import ChatBox from "@/components/user/ChatBox";
import LeadList from "@/components/user/LeadList";
import {
    Message,
    MessageType,
    MessageStatus,
    SenderRole,
    MessageAttachment,
} from "@/types";

// --- Interface ---
interface Design {
    id: number;
    img: string;
    name: string;
    designer: string;
    createdt: string;
    "type room": string;
}

interface CustomerLead {
    id: string;
    customerName: string;
    customerAvatar?: string;
    designRequests: Record<string, Design[]>;
    lastContactAt: string;
    totalMessages: number;
    budget?: number;
}

const ConsultationPage: React.FC = () => {
    const { user, profile } = useAuth();
    const [messages, setMessages] = useState<Message[]>([]);
    const [leads, setLeads] = useState<CustomerLead[]>([]);

    // ===============================
    // Load từ localStorage + xử lý pending designs
    // ===============================
    useEffect(() => {
        if (!user?.id || !profile) return;

        const savedLeads: CustomerLead[] = JSON.parse(localStorage.getItem(`leads_${user.id}`) || "[]");
        const savedMessages: Message[] = JSON.parse(localStorage.getItem(`messages_${user.id}`) || "[]");

        setLeads(savedLeads);
        setMessages(savedMessages);

        const pendingKey = `pendingDesigns_${user.id}`;
        const pendingDesigns: Design[] = JSON.parse(localStorage.getItem(pendingKey) || "[]");
        if (pendingDesigns.length === 0) return;

        setLeads(prevLeads => {
            const updatedLeads = [...prevLeads];
            // Kiểm tra Lead của khách hàng
            let lead = updatedLeads.find(l => l.customerName === profile.name);

            if (!lead) {
                // Nếu chưa có Lead → tạo Lead mới
                const grouped: Record<string, Design[]> = {};
                pendingDesigns.forEach(d => {
                    const type = d["type room"] || "Unknown Room";
                    if (!grouped[type]) grouped[type] = [];
                    grouped[type].push(d);
                });

                lead = {
                    id: "lead-" + Date.now(),
                    customerName: profile.name,
                    customerAvatar: profile.img,
                    designRequests: grouped,
                    lastContactAt: new Date().toISOString(),
                    totalMessages: 0,
                };
                updatedLeads.unshift(lead);
            } else {
                // Nếu đã có Lead → gom design mới theo type room, tránh trùng
                pendingDesigns.forEach(d => {
                    const type = d["type room"] || "Unknown Room";
                    if (!lead!.designRequests[type]) lead!.designRequests[type] = [];
                    const exists = lead!.designRequests[type].some(existing => existing.id === d.id);
                    if (!exists) lead!.designRequests[type].push(d);
                });
                lead.lastContactAt = new Date().toISOString();
            }

            localStorage.setItem(`leads_${user.id}`, JSON.stringify(updatedLeads));
            return updatedLeads;
        });

        // Chỉ tạo Message cho các design mới
        setMessages(prevMessages => {
            const designMessages: Message[] = [];

            pendingDesigns.forEach((design, idx) => {
                const exists = prevMessages.some(m =>
                    m.type === MessageType.DESIGN &&
                    m.attachments.some(att => att.id === design.id.toString())
                );
                if (!exists) {
                    designMessages.push({
                        id: Date.now().toString() + idx,
                        chatId: "consultation",
                        senderId: user.id,
                        senderRole: SenderRole.CUSTOMER,
                        senderName: profile.name,
                        senderAvatar: profile.img,
                        type: MessageType.DESIGN,
                        content: "",
                        attachments: [
                            {
                                id: design.id.toString(),
                                type: MessageType.DESIGN,
                                name: design.name,
                                url: design.img || "/placeholder.png",
                                thumbnailUrl: design.img || "/placeholder.png",
                                metadata: {
                                    designer: design.designer,
                                    roomType: design["type room"] || "Unknown Room",
                                    createdAt: design.createdt,
                                },
                            },
                        ],
                        status: MessageStatus.SENT,
                        isEdited: false,
                        isDeleted: false,
                        sentAt: new Date().toISOString(),
                        createdAt: new Date().toISOString(),
                        updatedAt: new Date().toISOString(),
                    });
                }
            });

            const updatedMessages = [...prevMessages, ...designMessages];
            localStorage.setItem(`messages_${user.id}`, JSON.stringify(updatedMessages));
            return updatedMessages;
        });

        localStorage.removeItem(pendingKey);

    }, [user, profile]);

    // ===============================
    // Gửi tin nhắn văn bản
    // ===============================
    const handleSendMessage = useCallback(
        (content: string, attachments?: File[] | MessageAttachment[]) => {
            if (!user || !profile) return;

            const newMessage: Message = {
                id: Date.now().toString() + Math.random(),
                chatId: "consultation",
                senderId: user.id,
                senderRole: SenderRole.CUSTOMER,
                senderName: profile.name,
                senderAvatar: profile.img,
                type: MessageType.TEXT,
                content,
                attachments:
                    attachments?.map((a: any, idx) =>
                        "id" in a
                            ? a
                            : {
                                id: Date.now().toString() + idx,
                                type: MessageType.FILE,
                                name: a.name,
                                url: URL.createObjectURL(a),
                            }
                    ) || [],
                status: MessageStatus.SENT,
                isEdited: false,
                isDeleted: false,
                sentAt: new Date().toISOString(),
                createdAt: new Date().toISOString(),
                updatedAt: new Date().toISOString(),
            };

            setMessages(prev => {
                const updated = [...prev, newMessage];
                localStorage.setItem(`messages_${user.id}`, JSON.stringify(updated));
                return updated;
            });
        },
        [user, profile]
    );

    // ===============================
    // UI
    // ===============================
    return (
        <div className="h-screen flex flex-col md:flex-row bg-gray-50">
            {/* Left: Lead List */}
            <div className="w-full md:w-96 border-b md:border-r border-gray-200 p-4 overflow-y-auto">
                <LeadList leads={leads} onLeadClick={(id) => console.log("Clicked lead", id)} />
            </div>

            {/* Right: Chat Box */}
            <div className="flex-1 p-4 flex flex-col overflow-hidden">
                <ChatBox
                    messages={messages}
                    currentUserId={user?.id || ""}
                    customerName={profile?.name || "You"}
                    onSendMessage={handleSendMessage}
                />
            </div>
        </div>
    );
};

export default ConsultationPage;
